...
//
var validate = {
	errorMessage:{'name':'','tel':'','mail':'','company':'','contents':''},
	reqArr:['name','tel','mail','contents'],									//必須項目のnameを格納
	chkArr:{'name':false,'tel':false,'mail':false,'contents':false},	//必須項目のnameを格納　デフォルト値としてfalse
	postReg:new RegExp('/^\d{3}-\d{4}$/'),
	enReg:new RegExp('/^[a-zA-Z]*$/'),
	init:function(){
		validate.addEvent();
		validate.reInputAlert();
		validate.submitAction();
	},
	addEvent:function(){
	...
	chk_num:function(str1){
		...
		if(str1.match(/^\d{10,11}$/)){	
			...
			validate.errorMessage['tel']='入力形式に誤りがあります。';
			$('.tel_div span.err_msg').text(validate.errorMessage['tel']);
			return false;
		}
	},
	...
	activateCheck:function(_this){
		var checkBtn = $('#check').prop('checked');
		var submitBtn = $('input[type="submit"]');
		(checkBtn) ? submitBtn.attr('disabled',false) : submitBtn.attr('disabled',true);
		(checkBtn) ? validate.inputDisabled() : validate.inputAbled();
	},
	submitAction:function(){
		$('input[type="submit"]').on('click',function(){
			var sts = $(this).prop('disabled');
			if(sts===true) return false;
			$('input, textarea').each(function(){
				$(this).attr('disabled', false);
			});
		});
